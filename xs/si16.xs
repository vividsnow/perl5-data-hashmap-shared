MODULE = Data::HashMap::Shared    PACKAGE = Data::HashMap::Shared::SI16
PROTOTYPES: DISABLE

SV*
new(char* class, SV* path_sv, UV max_entries, UV lru_max = 0, UV ttl_default = 0, UV lru_skip = 0, UV arena_cap = 0, UV file_mode = 0600)
    CODE:
        CK_U32(max_entries, "max_entries", "Data::HashMap::Shared::SI16"); CK_U32(lru_max, "lru_max", "Data::HashMap::Shared::SI16"); CK_U32(ttl_default, "ttl_default", "Data::HashMap::Shared::SI16"); CK_U32(lru_skip, "lru_skip", "Data::HashMap::Shared::SI16");
        char errbuf[SHM_ERR_BUFLEN]; const char* path = SHM_PATH_ARG(path_sv, "path", "Data::HashMap::Shared::SI16"); ShmHandle* map = shm_si16_create(path, (uint32_t)max_entries, (uint32_t)lru_max, (uint32_t)ttl_default, (uint32_t)lru_skip, (uint64_t)arena_cap, (mode_t)file_mode, errbuf);
        if (!map) croak("Data::HashMap::Shared::SI16: %s", errbuf[0] ? errbuf : "unknown error");
        CK_MAX_SIZE(map, "Data::HashMap::Shared::SI16");
        RETVAL = sv_setref_pv(newSV(0), class, (void*)map);
    OUTPUT:
        RETVAL

SV*
new_sharded(char* class, SV* path_prefix_sv, UV num_shards, UV max_entries, UV lru_max = 0, UV ttl_default = 0, UV lru_skip = 0, UV arena_cap = 0, UV file_mode = 0600)
    CODE:
        const char* path_prefix = SHM_PATH_ARG(path_prefix_sv, "path prefix", "Data::HashMap::Shared::SI16");
        CK_U32(max_entries, "max_entries", "Data::HashMap::Shared::SI16"); CK_U32(lru_max, "lru_max", "Data::HashMap::Shared::SI16"); CK_U32(ttl_default, "ttl_default", "Data::HashMap::Shared::SI16"); CK_U32(lru_skip, "lru_skip", "Data::HashMap::Shared::SI16");
        CK_U32(num_shards, "num_shards", "Data::HashMap::Shared::SI16");
        char errbuf[SHM_ERR_BUFLEN]; ShmHandle* map = shm_si16_create_sharded(path_prefix, (uint32_t)num_shards, (uint32_t)max_entries, (uint32_t)lru_max, (uint32_t)ttl_default, (uint32_t)lru_skip, (uint64_t)arena_cap, (mode_t)file_mode, errbuf);
        if (!map) croak("Data::HashMap::Shared::SI16: %s", errbuf[0] ? errbuf : "unknown error");
        CK_MAX_SIZE(map, "Data::HashMap::Shared::SI16");
        RETVAL = sv_setref_pv(newSV(0), class, (void*)map);
    OUTPUT:
        RETVAL


SV*
new_memfd(char* class, SV* name_sv, UV max_entries, UV lru_max = 0, UV ttl_default = 0, UV lru_skip = 0, UV arena_cap = 0)
    CODE:
        char errbuf[SHM_ERR_BUFLEN];
        const char* name = SHM_PATH_ARG(name_sv, "name", "Data::HashMap::Shared::SI16");
        CK_U32(max_entries, "max_entries", "Data::HashMap::Shared::SI16"); CK_U32(lru_max, "lru_max", "Data::HashMap::Shared::SI16"); CK_U32(ttl_default, "ttl_default", "Data::HashMap::Shared::SI16"); CK_U32(lru_skip, "lru_skip", "Data::HashMap::Shared::SI16");
        ShmHandle* map = shm_si16_create_memfd(name, (uint32_t)max_entries, (uint32_t)lru_max, (uint32_t)ttl_default, (uint32_t)lru_skip, (uint64_t)arena_cap, errbuf);
        if (!map) croak("Data::HashMap::Shared::SI16: %s", errbuf[0] ? errbuf : "unknown error");
        CK_MAX_SIZE(map, "Data::HashMap::Shared::SI16");
        RETVAL = sv_setref_pv(newSV(0), class, (void*)map);
    OUTPUT:
        RETVAL

SV*
new_from_fd(char* class, int fd)
    CODE:
        char errbuf[SHM_ERR_BUFLEN];
        ShmHandle* map = shm_si16_open_fd(fd, errbuf);
        if (!map) croak("Data::HashMap::Shared::SI16: %s", errbuf[0] ? errbuf : "unknown error");
        CK_MAX_SIZE(map, "Data::HashMap::Shared::SI16");
        RETVAL = sv_setref_pv(newSV(0), class, (void*)map);
    OUTPUT:
        RETVAL

SV*
new_readonly(char* class, SV* path_sv)
    CODE:
        char errbuf[SHM_ERR_BUFLEN];
        const char* path = SHM_PATH_ARG(path_sv, "path", "Data::HashMap::Shared::SI16");
        if (!path) croak("Data::HashMap::Shared::SI16->new_readonly: path is required");
        ShmHandle* map = shm_si16_open_readonly(path, errbuf);
        if (!map) croak("Data::HashMap::Shared::SI16: %s", errbuf[0] ? errbuf : "unknown error");
        RETVAL = sv_setref_pv(newSV(0), class, (void*)map);
    OUTPUT:
        RETVAL

void
freeze(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16->freeze: cannot freeze a read-only handle");
        if (shm_freeze(h) != 0) croak("Data::HashMap::Shared::SI16->freeze: msync: %s", strerror(errno));
        shm_mark_readonly(h);

UV
frozen(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = (UV)((h->shard_handles ? h->shard_handles[0]->hdr->sealed : h->hdr->sealed) ? 1 : 0);
    OUTPUT:
        RETVAL

UV
readonly(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = (UV)(h->readonly ? 1 : 0);
    OUTPUT:
        RETVAL

IV
memfd(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = h->backing_fd;
    OUTPUT:
        RETVAL


void
sync(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (shm_msync(h) != 0) croak("Data::HashMap::Shared::SI16 sync: %s", strerror(errno));

void
DESTROY(SV* self_sv)
    CODE:
        if (!sv_isobject(self_sv) || !sv_derived_from(self_sv, "Data::HashMap::Shared::SI16")) return;
        ShmHandle* h = INT2PTR(ShmHandle*, SvIV(SvRV(self_sv)));
        if (!h) return;
        sv_setiv(SvRV(self_sv), 0);
        shm_close_map(h);

bool
put(SV* self_sv, SV* key_sv, int16_t value)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = shm_si16_put(h, _kstr, (uint32_t)_klen, _kutf8, value);
    OUTPUT:
        RETVAL

UV
set_multi(SV* self_sv, ...)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        if ((items - 1) % 2 != 0) croak("set_multi requires even number of arguments (key, value pairs)");
        uint32_t count = 0;
        if (h->shard_handles) {
            for (int i = 1; i < items; i += 2) {
                int16_t _val = (int16_t)SvIV(ST(i + 1));
                REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
                STRLEN _klen; const char *_kstr = SvPV(ST(i), _klen);
                bool _kutf8 = SvUTF8(ST(i)) ? 1 : 0;
                if (_klen > SHM_MAX_STR_LEN) croak("key too long (max 1GB)");
                REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
                count += shm_si16_put(h, _kstr, (uint32_t)_klen, _kutf8, _val);
            }
        } else {
            WRSEQ_GUARD(h);
            for (int i = 1; i < items; i += 2) {
                int16_t _val = (int16_t)SvIV(ST(i + 1));
                REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
                STRLEN _klen; const char *_kstr = SvPV(ST(i), _klen);
                bool _kutf8 = SvUTF8(ST(i)) ? 1 : 0;
                if (_klen > SHM_MAX_STR_LEN) croak("key too long (max 1GB)");
                REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
                count += shm_si16_put_inner(h, _kstr, (uint32_t)_klen, _kutf8, _val, SHM_TTL_USE_DEFAULT);
            }
        }
        RETVAL = count;
    OUTPUT:
        RETVAL

UV
remove_multi(SV* self_sv, ...)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        uint32_t count = 0;
        if (h->shard_handles) {
            for (int i = 1; i < items; i++) {
                STRLEN _kl; const char *_ks = SvPV(ST(i), _kl);
                bool _ku = SvUTF8(ST(i)) ? 1 : 0;
                if (_kl > SHM_MAX_STR_LEN) croak("key too long (max 1GB)");
                REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
                count += shm_si16_remove(h, _ks, (uint32_t)_kl, _ku);
            }
        } else {
            WRSEQ_GUARD(h);
            for (int i = 1; i < items; i++) {
                STRLEN _kl; const char *_ks = SvPV(ST(i), _kl);
                bool _ku = SvUTF8(ST(i)) ? 1 : 0;
                if (_kl > SHM_MAX_STR_LEN) croak("key too long (max 1GB)");
                REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
                count += shm_si16_remove_inner(h, _ks, (uint32_t)_kl, _ku);
            }
            if (count) shm_si16_maybe_shrink(h);
        }
        RETVAL = count;
    OUTPUT:
        RETVAL

void
get_multi(SV* self_sv, ...)
    PPCODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        int nkeys = items - 1;
        if (nkeys == 0) XSRETURN_EMPTY;
        EXTEND(SP, nkeys);
        if (h->shard_handles) {
            for (int i = 0; i < nkeys; i++) {
                STRLEN _kl; const char *_ks = SvPV(ST(i + 1), _kl);
                bool _ku = SvUTF8(ST(i + 1)) ? 1 : 0;
                int16_t val;
                REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
                if (shm_si16_get(h, _ks, (uint32_t)_kl, _ku, &val))
                    mPUSHi(val);
                else
                    PUSHs(&PL_sv_undef);
            }
        } else {
            ShmHeader *hdr = h->hdr;
            ShmNodeSI16 *nodes = (ShmNodeSI16 *)h->nodes;
            uint8_t *states = h->states;
            char *arena = h->arena;
            uint32_t now = h->expires_at ? shm_now() : 0;
            RDLOCK_GUARD(h);
            uint32_t mask = hdr->table_cap - 1;
            for (int i = 0; i < nkeys; i++) {
                STRLEN _kl; const char *_ks = SvPV(ST(i + 1), _kl);
                bool _ku = SvUTF8(ST(i + 1)) ? 1 : 0;
                REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
                uint32_t hash = shm_hash_string(_ks, (uint32_t)_kl);
                uint32_t pos = hash & mask;
                uint8_t tag = SHM_MAKE_TAG(hash);
                int found = 0;
                uint32_t fidx = 0;
                int16_t val = 0;
                for (uint32_t j = 0; j <= mask; j++) {
                    uint32_t idx = (pos + j) & mask;
                    uint8_t st = states[idx];
                    if (st == SHM_EMPTY) break;
                    if (st != tag) continue;
                    if (shm_si16__key_eq_str(&nodes[idx], arena, _ks, (uint32_t)_kl, _ku)) {
                        if (h->expires_at && h->expires_at[idx] && now >= h->expires_at[idx]) break;
                        val = __atomic_load_n(&nodes[idx].value, __ATOMIC_RELAXED);
                        fidx = idx;
                        found = 1; break;
                    }
                }
                if (found) {
                    /* count as an access for LRU second-chance, like get() does */
                    if (h->lru_accessed && !h->readonly)
                        __atomic_store_n(&h->lru_accessed[fidx], 1, __ATOMIC_RELAXED);
                    mPUSHi(val);
                } else PUSHs(&PL_sv_undef);
                /* Prefetch next key's probe start */
                if (i + 1 < nkeys) {
                    STRLEN nkl; const char *nks = SvPV(ST(i + 2), nkl);
                    uint32_t nh = shm_hash_string(nks, (uint32_t)nkl);
                    __builtin_prefetch(&states[nh & mask], 0, 0);
                }
            }
        }

void
get_with_ttl(SV* self_sv, SV* key_sv)
    PPCODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        EXTRACT_STR_KEY(key_sv);
        int16_t out_value;
        int64_t out_ttl;
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (!shm_si16_get_with_ttl(h, _kstr, (uint32_t)_klen, _kutf8, &out_value, &out_ttl)) XSRETURN_EMPTY;
        EXTEND(SP, 2);
        mPUSHi(out_value);
        if (out_ttl < 0) PUSHs(&PL_sv_undef);
        else mPUSHi(out_ttl);

SV*
stats(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        HV *hv = newHV();
        hv_store(hv, "size", 4, newSVuv(shm_si16_size(h)), 0);
        hv_store(hv, "capacity", 8, newSVuv(shm_si16_capacity(h)), 0);
        hv_store(hv, "max_entries", 11, newSVuv(shm_si16_max_entries(h)), 0);
        hv_store(hv, "tombstones", 10, newSVuv(shm_si16_tombstones(h)), 0);
        hv_store(hv, "mmap_size", 9, newSVuv(shm_si16_mmap_size(h)), 0);
        hv_store(hv, "arena_used", 10, newSVuv(shm_si16_arena_used(h)), 0);
        hv_store(hv, "arena_cap", 9, newSVuv(shm_si16_arena_cap(h)), 0);
        hv_store(hv, "evictions", 9, newSVuv(shm_si16_stat_evictions(h)), 0);
        hv_store(hv, "expired", 7, newSVuv(shm_si16_stat_expired(h)), 0);
        hv_store(hv, "recoveries", 10, newSVuv(shm_si16_stat_recoveries(h)), 0);
        hv_store(hv, "max_size", 8, newSVuv(shm_si16_max_size(h)), 0);
        hv_store(hv, "ttl", 3, newSVuv(shm_si16_ttl(h)), 0);
        hv_store(hv, "frozen", 6, newSVuv((UV)((h->shard_handles ? h->shard_handles[0]->hdr->sealed : h->hdr->sealed) ? 1 : 0)), 0);
        hv_store(hv, "readonly", 8, newSVuv((UV)(h->readonly ? 1 : 0)), 0);
        RETVAL = newRV_noinc((SV*)hv);
    OUTPUT:
        RETVAL

bool
cas(SV* self_sv, SV* key_sv, int16_t expected, int16_t desired)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = shm_si16_cas(h, _kstr, (uint32_t)_klen, _kutf8, expected, desired);
    OUTPUT:
        RETVAL

SV*
cas_take(SV* self_sv, SV* key_sv, int16_t expected)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        int16_t out_value;
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (!shm_si16_cas_take(h, _kstr, (uint32_t)_klen, _kutf8, expected, &out_value)) XSRETURN_UNDEF;
        RETVAL = newSViv(out_value);
    OUTPUT:
        RETVAL

bool
persist(SV* self_sv, SV* key_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = shm_si16_persist(h, _kstr, (uint32_t)_klen, _kutf8);
    OUTPUT:
        RETVAL

bool
set_ttl(SV* self_sv, SV* key_sv, UV ttl_sec)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        CK_U32(ttl_sec, "ttl", "Data::HashMap::Shared::SI16");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = shm_si16_set_ttl(h, _kstr, (uint32_t)_klen, _kutf8, (uint32_t)ttl_sec);
    OUTPUT:
        RETVAL

bool
put_ttl(SV* self_sv, SV* key_sv, int16_t value, UV ttl_sec)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        CK_U32(ttl_sec, "ttl", "Data::HashMap::Shared::SI16");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        REQUIRE_TTL(h);
        RETVAL = shm_si16_put_ttl(h, _kstr, (uint32_t)_klen, _kutf8, value, (uint32_t)ttl_sec);
    OUTPUT:
        RETVAL

UV
max_size(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = (UV)shm_si16_max_size(h);
    OUTPUT:
        RETVAL

UV
ttl(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = (UV)shm_si16_ttl(h);
    OUTPUT:
        RETVAL

SV*
get(SV* self_sv, SV* key_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        EXTRACT_STR_KEY(key_sv);
        int16_t value;
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (!shm_si16_get(h, _kstr, (uint32_t)_klen, _kutf8, &value)) XSRETURN_UNDEF;
        RETVAL = newSViv(value);
    OUTPUT:
        RETVAL

bool
remove(SV* self_sv, SV* key_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = shm_si16_remove(h, _kstr, (uint32_t)_klen, _kutf8);
    OUTPUT:
        RETVAL

bool
exists(SV* self_sv, SV* key_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = shm_si16_exists(h, _kstr, (uint32_t)_klen, _kutf8);
    OUTPUT:
        RETVAL

SV*
incr(SV* self_sv, SV* key_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        int ok;
        int16_t val = shm_si16_incr_by(h, _kstr, (uint32_t)_klen, _kutf8, 1, &ok);
        if (!ok) croak("Data::HashMap::Shared::SI16: increment failed");
        RETVAL = newSViv(val);
    OUTPUT:
        RETVAL

SV*
decr(SV* self_sv, SV* key_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        int ok;
        int16_t val = shm_si16_incr_by(h, _kstr, (uint32_t)_klen, _kutf8, -1, &ok);
        if (!ok) croak("Data::HashMap::Shared::SI16: decrement failed");
        RETVAL = newSViv(val);
    OUTPUT:
        RETVAL

SV*
incr_by(SV* self_sv, SV* key_sv, int16_t delta)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        int ok;
        int16_t val = shm_si16_incr_by(h, _kstr, (uint32_t)_klen, _kutf8, delta, &ok);
        if (!ok) croak("Data::HashMap::Shared::SI16: incr_by failed");
        RETVAL = newSViv(val);
    OUTPUT:
        RETVAL

SV*
max(SV* self_sv, SV* key_sv, int16_t desired)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        int ok;
        int16_t val = shm_si16_set_minmax(h, _kstr, (uint32_t)_klen, _kutf8, desired, 1, &ok);
        if (!ok) croak("Data::HashMap::Shared::SI16: max failed");
        RETVAL = newSViv(val);
    OUTPUT:
        RETVAL

SV*
min(SV* self_sv, SV* key_sv, int16_t desired)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        int ok;
        int16_t val = shm_si16_set_minmax(h, _kstr, (uint32_t)_klen, _kutf8, desired, 0, &ok);
        if (!ok) croak("Data::HashMap::Shared::SI16: min failed");
        RETVAL = newSViv(val);
    OUTPUT:
        RETVAL

UV
size(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = (UV)shm_si16_size(h);
    OUTPUT:
        RETVAL

UV
max_entries(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = (UV)shm_si16_max_entries(h);
    OUTPUT:
        RETVAL

void
keys(SV* self_sv)
    PPCODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        uint32_t ns = h->shard_handles ? h->num_shards : 1;
        for (uint32_t si = 0; si < ns; si++) {
            ShmHandle *sh = h->shard_handles ? h->shard_handles[si] : h;
            ShmHeader *hdr = sh->hdr;
            ShmNodeSI16 *nodes = (ShmNodeSI16 *)sh->nodes;
            uint32_t now = sh->expires_at ? shm_now() : 0;
            RDLOCK_GUARD(sh);
            EXTEND(SP, hdr->size);
            for (uint32_t i = 0; i < hdr->table_cap; i++) {
                if (SHM_IS_LIVE(sh->states[i]) && !SHM_IS_EXPIRED(sh, i, now)) {
                    char _ib[SHM_INLINE_MAX]; uint32_t klen;
                    const char *kp = shm_str_ptr(nodes[i].key_off, nodes[i].key_len, sh->arena, sh->hdr->arena_cap, _ib, &klen);
                    SV* sv = newSVpvn(kp, klen);
                    if (SHM_UNPACK_UTF8(nodes[i].key_len)) SvUTF8_on(sv);
                    mXPUSHs(sv);
                }
            }
        }

void
values(SV* self_sv)
    PPCODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        uint32_t ns = h->shard_handles ? h->num_shards : 1;
        for (uint32_t si = 0; si < ns; si++) {
            ShmHandle *sh = h->shard_handles ? h->shard_handles[si] : h;
            ShmHeader *hdr = sh->hdr;
            ShmNodeSI16 *nodes = (ShmNodeSI16 *)sh->nodes;
            uint32_t now = sh->expires_at ? shm_now() : 0;
            RDLOCK_GUARD(sh);
            EXTEND(SP, hdr->size);
            for (uint32_t i = 0; i < hdr->table_cap; i++) {
                if (SHM_IS_LIVE(sh->states[i]) && !SHM_IS_EXPIRED(sh, i, now))
                    mXPUSHi(__atomic_load_n(&nodes[i].value, __ATOMIC_RELAXED));
            }
        }

void
items(SV* self_sv)
    PPCODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        uint32_t ns = h->shard_handles ? h->num_shards : 1;
        for (uint32_t si = 0; si < ns; si++) {
            ShmHandle *sh = h->shard_handles ? h->shard_handles[si] : h;
            ShmHeader *hdr = sh->hdr;
            ShmNodeSI16 *nodes = (ShmNodeSI16 *)sh->nodes;
            uint32_t now = sh->expires_at ? shm_now() : 0;
            RDLOCK_GUARD(sh);
            EXTEND(SP, hdr->size * 2);
            for (uint32_t i = 0; i < hdr->table_cap; i++) {
                if (SHM_IS_LIVE(sh->states[i]) && !SHM_IS_EXPIRED(sh, i, now)) {
                    char _ib[SHM_INLINE_MAX]; uint32_t klen;
                    const char *kp = shm_str_ptr(nodes[i].key_off, nodes[i].key_len, sh->arena, sh->hdr->arena_cap, _ib, &klen);
                    SV* sv = newSVpvn(kp, klen);
                    if (SHM_UNPACK_UTF8(nodes[i].key_len)) SvUTF8_on(sv);
                    mXPUSHs(sv);
                    mXPUSHi(__atomic_load_n(&nodes[i].value, __ATOMIC_RELAXED));
                }
            }
        }


void
each(SV* self_sv)
    PPCODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        const char *out_key; uint32_t out_klen; bool out_kutf8;
        int16_t out_value;
        if (shm_si16_each(h, &out_key, &out_klen, &out_kutf8, &out_value)) {
            EXTEND(SP, 2);
            SV* ksv = newSVpvn(out_key, out_klen);
            if (out_kutf8) SvUTF8_on(ksv);
            mXPUSHs(ksv);
            mXPUSHi(out_value);
            XSRETURN(2);
        }
        shm_si16_flush_deferred(h);
        XSRETURN_EMPTY;

void
iter_reset(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        shm_si16_iter_reset(h);
        shm_si16_flush_deferred(h);

void
clear(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        shm_si16_clear(h);

SV*
to_hash(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        HV* hv = newHV();
        uint32_t ns = h->shard_handles ? h->num_shards : 1;
        for (uint32_t si = 0; si < ns; si++) {
            ShmHandle *sh = h->shard_handles ? h->shard_handles[si] : h;
            ShmHeader *hdr = sh->hdr;
            ShmNodeSI16 *nodes = (ShmNodeSI16 *)sh->nodes;
            uint32_t now = sh->expires_at ? shm_now() : 0;
            RDLOCK_GUARD(sh);
            for (uint32_t i = 0; i < hdr->table_cap; i++) {
                if (SHM_IS_LIVE(sh->states[i]) && !SHM_IS_EXPIRED(sh, i, now)) {
                    char _ib[SHM_INLINE_MAX]; uint32_t klen;
                    const char *kp = shm_str_ptr(nodes[i].key_off, nodes[i].key_len, sh->arena, sh->hdr->arena_cap, _ib, &klen);
                    bool kutf8 = SHM_UNPACK_UTF8(nodes[i].key_len);
                    SV* val = newSViv(__atomic_load_n(&nodes[i].value, __ATOMIC_RELAXED));
                    if (!hv_store(hv, kp,
                                   kutf8 ? -(I32)klen : (I32)klen, val, 0)) SvREFCNT_dec(val);
                }
            }
        }

        RETVAL = newRV_noinc((SV*)hv);
    OUTPUT:
        RETVAL

SV*
get_or_set(SV* self_sv, SV* key_sv, int16_t default_value)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        int16_t out;
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        int rc = shm_si16_get_or_set(h, _kstr, (uint32_t)_klen, _kutf8, default_value, &out);
        if (!rc) XSRETURN_UNDEF;
        RETVAL = newSViv(out);
    OUTPUT:
        RETVAL

SV*
take(SV* self_sv, SV* key_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        int16_t out_value;
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (!shm_si16_take(h, _kstr, (uint32_t)_klen, _kutf8, &out_value)) XSRETURN_UNDEF;
        RETVAL = newSViv(out_value);
    OUTPUT:
        RETVAL

void
pop(SV* self_sv)
    PPCODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        const char *out_key; uint32_t out_klen; bool out_kutf8;
        int16_t out_val;
        if (!shm_si16_pop(h, &out_key, &out_klen, &out_kutf8, &out_val)) XSRETURN_EMPTY;
        EXTEND(SP, 2);
        SV *ksv = newSVpvn(out_key, out_klen);
        if (out_kutf8) SvUTF8_on(ksv);
        mPUSHs(ksv);
        mPUSHi(out_val);

void
shift(SV* self_sv)
    PPCODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        const char *out_key; uint32_t out_klen; bool out_kutf8;
        int16_t out_val;
        if (!shm_si16_shift(h, &out_key, &out_klen, &out_kutf8, &out_val)) XSRETURN_EMPTY;
        EXTEND(SP, 2);
        SV *ksv = newSVpvn(out_key, out_klen);
        if (out_kutf8) SvUTF8_on(ksv);
        mPUSHs(ksv);
        mPUSHi(out_val);

void
drain(SV* self_sv, UV limit)
    PPCODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        /* Only as many as the map can actually yield: drain(1e9) on a
         * ten-entry map otherwise reserved a billion entries up front. */
        {
            UV avail = (UV)shm_si16_size(h);
            if (limit > avail) limit = avail;
        }
        if (limit == 0) XSRETURN_EMPTY;
        shm_si16_drain_entry *entries;
        Newxz(entries, limit, shm_si16_drain_entry);

        SAVEFREEPV(entries);
        char *buf = NULL; uint32_t buf_cap = 0;
        uint32_t n = shm_si16_drain(h, (uint32_t)limit, entries, &buf, &buf_cap);
        if (buf) SAVEDESTRUCTOR_X(shm_free_cleanup, buf);

        EXTEND(SP, n * 2);
        for (uint32_t i = 0; i < n; i++) {
            SV *ksv = newSVpvn(buf + entries[i].key_off, entries[i].key_len);
            if (entries[i].key_utf8) SvUTF8_on(ksv);
            mPUSHs(ksv);
            mPUSHi(entries[i].value);
        }


UV
flush_expired(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        RETVAL = (UV)shm_si16_flush_expired(h);
    OUTPUT:
        RETVAL

void
flush_expired_partial(SV* self_sv, UV limit)
    PPCODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        CK_U32(limit, "limit", "Data::HashMap::Shared::SI16");
        int done = 0;
        uint32_t flushed = shm_si16_flush_expired_partial(h, (uint32_t)limit, &done);
        EXTEND(SP, 2);
        mPUSHu(flushed);
        mPUSHi(done);

UV
mmap_size(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = (UV)shm_si16_mmap_size(h);
    OUTPUT:
        RETVAL

bool
touch(SV* self_sv, SV* key_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = shm_si16_touch(h, _kstr, (uint32_t)_klen, _kutf8);
    OUTPUT:
        RETVAL

bool
reserve(SV* self_sv, UV target)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        CK_U32(target, "target", "Data::HashMap::Shared::SI16");
        RETVAL = shm_si16_reserve(h, (uint32_t)target);
    OUTPUT:
        RETVAL

UV
stat_evictions(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = (UV)shm_si16_stat_evictions(h);
    OUTPUT:
        RETVAL

UV
stat_expired(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = (UV)shm_si16_stat_expired(h);
    OUTPUT:
        RETVAL

UV
stat_recoveries(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = (UV)shm_si16_stat_recoveries(h);
    OUTPUT:
        RETVAL

UV
arena_used(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = (UV)shm_si16_arena_used(h);
    OUTPUT:
        RETVAL

UV
arena_cap(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = (UV)shm_si16_arena_cap(h);
    OUTPUT:
        RETVAL

bool
add(SV* self_sv, SV* key_sv, int16_t value)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = shm_si16_add(h, _kstr, (uint32_t)_klen, _kutf8, value);
    OUTPUT:
        RETVAL

bool
add_ttl(SV* self_sv, SV* key_sv, int16_t value, UV ttl_sec)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        CK_U32(ttl_sec, "ttl", "Data::HashMap::Shared::SI16");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        REQUIRE_TTL(h);
        RETVAL = shm_si16_add_ttl(h, _kstr, (uint32_t)_klen, _kutf8, value, (uint32_t)ttl_sec);
    OUTPUT:
        RETVAL

bool
update(SV* self_sv, SV* key_sv, int16_t value)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = shm_si16_update(h, _kstr, (uint32_t)_klen, _kutf8, value);
    OUTPUT:
        RETVAL

bool
update_ttl(SV* self_sv, SV* key_sv, int16_t value, UV ttl_sec)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        CK_U32(ttl_sec, "ttl", "Data::HashMap::Shared::SI16");
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        REQUIRE_TTL(h);
        RETVAL = shm_si16_update_ttl(h, _kstr, (uint32_t)_klen, _kutf8, value, (uint32_t)ttl_sec);
    OUTPUT:
        RETVAL

SV*
swap(SV* self_sv, SV* key_sv, int16_t value)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI16: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
        int16_t out_value;
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        int rc = shm_si16_swap(h, _kstr, (uint32_t)_klen, _kutf8, value, &out_value);
        if (rc != 1) XSRETURN_UNDEF;
        RETVAL = newSViv(out_value);
    OUTPUT:
        RETVAL

SV*
path(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = h->path ? newSVpv(h->path, 0) : &PL_sv_undef;
    OUTPUT:
        RETVAL

bool
unlink(SV* self_or_class, ...)
    CODE:
        if (sv_isobject(self_or_class) && sv_derived_from(self_or_class, "Data::HashMap::Shared::SI16")) {
            ShmHandle* h = INT2PTR(ShmHandle*, SvIV(SvRV(self_or_class)));
            if (!h) croak("Attempted to use a destroyed Data::HashMap::Shared::SI16 object");
            RETVAL = shm_unlink_sharded(h);
        } else {
            if (items < 2) croak("Usage: Data::HashMap::Shared::SI16->unlink($path)");
            RETVAL = shm_unlink_path(shm_path_arg(aTHX_ ST(1), "path", "Data::HashMap::Shared::SI16"));
        }
    OUTPUT:
        RETVAL

SV*
ttl_remaining(SV* self_sv, SV* key_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        int64_t remaining = shm_si16_ttl_remaining(h, _kstr, (uint32_t)_klen, _kutf8);
        if (remaining < 0) XSRETURN_UNDEF;
        RETVAL = newSViv(remaining);
    OUTPUT:
        RETVAL

UV
capacity(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = (UV)shm_si16_capacity(h);
    OUTPUT:
        RETVAL

UV
tombstones(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        RETVAL = (UV)shm_si16_tombstones(h);
    OUTPUT:
        RETVAL

SV*
cursor(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI16", self_sv);
        ShmCursor* c = shm_cursor_create(h);
        if (!c) croak("Failed to allocate cursor");
        c->owner = SvREFCNT_inc(SvRV(self_sv));
        RETVAL = sv_setref_pv(newSV(0), "Data::HashMap::Shared::SI16::Cursor", (void*)c);
    OUTPUT:
        RETVAL

MODULE = Data::HashMap::Shared    PACKAGE = Data::HashMap::Shared::SI16::Cursor
PROTOTYPES: DISABLE

void
DESTROY(SV* self_sv)
    CODE:
        if (!sv_isobject(self_sv) || !sv_derived_from(self_sv, "Data::HashMap::Shared::SI16::Cursor")) return;
        ShmCursor* c = INT2PTR(ShmCursor*, SvIV(SvRV(self_sv)));
        if (!c) return;
        CURSOR_DETACH_IF_MAP_GONE(c, owner, h);
        shm_cursor_destroy(c);
        if (h) shm_si16_flush_deferred(h);
        if (owner) SvREFCNT_dec(owner);
        sv_setiv(SvRV(self_sv), 0);

void
next(SV* self_sv)
    PPCODE:
        EXTRACT_CURSOR("Data::HashMap::Shared::SI16::Cursor", self_sv);
        const char *out_key; uint32_t out_klen; bool out_kutf8;
        int16_t out_value;
        if (shm_si16_cursor_next(c, &out_key, &out_klen, &out_kutf8, &out_value)) {
            EXTEND(SP, 2);
            SV* ksv = newSVpvn(out_key, out_klen);
            if (out_kutf8) SvUTF8_on(ksv);
            mXPUSHs(ksv);
            mXPUSHi(out_value);
            XSRETURN(2);
        }
        XSRETURN_EMPTY;

void
reset(SV* self_sv)
    CODE:
        EXTRACT_CURSOR("Data::HashMap::Shared::SI16::Cursor", self_sv);
        shm_si16_cursor_reset(c);

bool
seek(SV* self_sv, SV* key_sv)
    CODE:
        EXTRACT_CURSOR("Data::HashMap::Shared::SI16::Cursor", self_sv);
        EXTRACT_STR_KEY(key_sv);
        REEXTRACT_CURSOR("Data::HashMap::Shared::SI16::Cursor", self_sv);
        RETVAL = shm_si16_cursor_seek(c, _kstr, (uint32_t)_klen, _kutf8);
    OUTPUT:
        RETVAL

